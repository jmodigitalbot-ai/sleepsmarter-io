...
export default function EmailCapture({ calculatorData }: EmailCaptureProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (calculatorData) {
      payload.fields = {
        ...
        optimal_time: calculatorData.results.find(r => r.quality === 'optimal')?.time || '',
        cycles_preferred: calculatorData.results.find(r => r.quality === 'optimal')?.cycles || 6
      }
    }
  ...
  return (
    <div className="mt-6 bg-[#1a1a2e] border border-[#4a4e69]/30 rounded-xl p-6">
      ...
      {process.env.NODE_ENV === 'development' && calculatorData && (
